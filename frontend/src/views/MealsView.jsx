...
function MarkCookedModal({ meal, members, onClose, onSaved }) {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            onSaved(meal, cookedBy ? parseInt(cookedBy) : null);
            onClose();
        } catch (err) {
            alert(err.message);
        } finally {
...
function PlanTab({ meals, loading, members, savedRecipes, ratingSummaries, preferences, favRecipeIds, activeMemberId, onRefresh, onMealCooked }) {
    ...
    const selectRecipe = (recipe) => {
        setForm(f => ({
            ...
            recipe_id: recipe.id,
            ingredients: (recipe.ingredients || []).map(i => i.ingredient_name || i).join(', '),
    ...
    const toggleFavorite = async (recipeId) => {
        if (!recipeId || !activeMemberId) return;
        try {
            await post(`/recipes/${recipeId}/favorite?member_id=${activeMemberId}`);
            await onRefresh();
        } catch (e) { console.error(e); }
    };

    let mealList = meals?.meals || [];
    if (activeFilter === 'Favorites') {
        mealList = mealList.filter(m => m.recipe_id && favRecipeIds.has(m.recipe_id));
    }
    ...
    return (
        <>
                ...
                <div className="flex gap-1 bg-surface-800/50 rounded-xl p-1">
                    {['All', 'Favorites'].map(f => (
                        <button key={f} onClick={() => setActiveFilter(f)}
                            className={`px-3 py-1.5 rounded-lg text-sm font-medium transition-colors min-h-[36px] active:scale-[0.97]
                                ${activeFilter === f ? 'bg-forest-600 text-white' : 'text-surface-400 hover:text-surface-200'}`}>
                            {f === 'Favorites' ? '❤️ ' : ''}{f}
                        </button>
                    ))}
                </div>
                        ...
                        <div className="flex gap-2">
                            ...
                            {savedRecipes.length > 0 && (
                                <button type="button" onClick={() => setShowRecipePicker(true)}
                                    className="px-3 py-2.5 bg-ocean-600/20 border border-ocean-600/30 text-ocean-300 hover:bg-ocean-600/30 rounded-xl text-sm active:scale-[0.97]">
                                    Recipes
                                </button>
                            )}
                        </div>
            ...
            {/* Recipe picker modal */}
            {showRecipePicker && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm" onClick={() => setShowRecipePicker(false)}>
                    <div className="bg-surface-800 rounded-2xl p-6 max-w-md w-full mx-4 max-h-[70vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
                        <h2 className="text-lg font-bold text-surface-100 mb-4">Choose a Recipe</h2>
                        <div className="space-y-2">
                            {savedRecipes.map(r => (
                                <button key={r.id} onClick={() => selectRecipe(r)}
                                    className="w-full text-left px-4 py-3 bg-surface-700 hover:bg-surface-600 rounded-xl transition-colors active:scale-[0.98]">
                                    <p className="font-medium text-surface-100">{r.name}</p>
                                    <p className="text-xs text-surface-400 mt-0.5">{r.category} · {r.prep_time_min}m prep</p>
                                </button>
                            ))}
                        </div>
                        <button onClick={() => setShowRecipePicker(false)}
                            className="mt-4 w-full py-2.5 bg-surface-700 text-surface-300 rounded-xl text-sm active:scale-[0.97]">
                            Cancel
                        </button>
                    </div>
                </div>
            ...
            {loading ? (
                ...
                <div className="text-center py-12 text-surface-400">
                    {activeFilter === 'Favorites' ? 'No favorite meals this week.' : 'No meals planned. Add one above!'}
                </div>
            ) : (
                <div className="space-y-3">
                    {mealList.map(meal => {
                        const rSummary = meal.recipe_id ? ratingSummaries[meal.recipe_id] : null;
                        const mPrefs = meal.recipe_id ? (preferences[meal.recipe_id] || []) : [];
                        const isFav = meal.recipe_id && favRecipeIds.has(meal.recipe_id);
                        const isExpanded = expandedId === meal.id;
                        return (
                            <div key={meal.id} className="bg-surface-800 rounded-xl overflow-hidden">
                                <div
                                    className={`flex items-center gap-4 px-5 py-4 cursor-pointer transition-colors
                                        ${editingId === meal.id ? 'ring-2 ring-forest-500' : 'hover:bg-surface-750'}`}
                                    ...
                                    <div className="flex-1 min-w-0">
                                        ...
                                        {rSummary?.average && (
                                            <p className="text-xs text-amber-400 mt-0.5">★ {rSummary.average} ({rSummary.count})</p>
                                        )}
                                    </div>
                                    ...
                                    {meal.recipe_id && (
                                        <button
                                            ...
                                            className="text-surface-500 hover:text-ocean-400 text-sm w-9 h-9 flex items-center justify-center rounded-xl hover:bg-surface-700/50 active:scale-[0.95]">
                                            {isExpanded ? '▲' : '▼'}
                                        </button>
                                    )}
                                    <button
                                ...
                                {/* Expanded ratings / preferences panel */}
                                {isExpanded && meal.recipe_id && (
                                    <div className="px-5 pb-4 border-t border-surface-700 pt-3 space-y-2">
                                        {mPrefs.length > 0 && <PreferenceBadges preferences={mPrefs} members={members} />}
                                        <div className="flex items-center gap-3">
                                            <span className="text-xs text-surface-400">Your rating:</span>
                                            <StarRating
                                                recipeId={meal.recipe_id}
                                                memberId={activeMemberId}
                                                existing={mPrefs.find(p => p.member_id === activeMemberId)}
                                            />
                                            <button onClick={(e) => { e.stopPropagation(); toggleFavorite(meal.recipe_id); }}
                                                className={`ml-auto text-sm px-3 py-1.5 rounded-xl transition-colors active:scale-[0.97]
                                                    ${isFav ? 'bg-rose-600/20 border border-rose-600/30 text-rose-300' : 'bg-surface-700 text-surface-400 hover:text-rose-400'}`}>
            ...
            {markingMeal && (
                <MarkCookedModal
                    ...
                    onSaved={(meal, cookerId) => { onMealCooked(meal, cookerId); }}
                />
            )}
        </>
    );
...
export default function MealsView() {
    ...
    const [favRecipeIds, setFavRecipeIds] = useState(new Set());

    // "Rate after cooking" modal
    const [ratingMeal, setRatingMeal] = useState(null);

    const loadRecipeData = async (recipeIds) => {
        const summaries = {};
        const prefs = {};
        ...
        await Promise.allSettled(
            recipeIds.flatMap(rid => [
                ...
                get(`/recipes/${rid}/preferences`).then(p => {
                    prefs[rid] = p;
                    if (p.some(pref => pref.is_favorite)) favIds.add(rid);
                }),
            ])
        );
    ...
    const loadMeals = async () => {
        try {
            ...
            setMeals(data);
            const recipeIds = [...new Set((data.meals || []).map(x => x.recipe_id).filter(Boolean))];
            if (recipeIds.length > 0) loadRecipeData(recipeIds);
        } catch (e) { console.error(e); }
    };
    ...
    const handleMealCooked = (meal, cookerId) => {
        if (meal.recipe_id) {
            setRatingMeal({ meal, cookerId });
        }
        loadMeals();
    };
        ...
        <div className="min-h-screen p-4 md:p-8 max-w-4xl mx-auto">
            ...
            {ratingMeal && (
                <RateAfterCookingModal
                    meal={ratingMeal.meal}
                    cookerId={ratingMeal.cookerId}
                    members={members}
